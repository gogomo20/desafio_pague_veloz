...
public class DebitCommandTest
{
    ...
    public async Task Deve_Executar_Com_Sucesso_Quando_Dados_Forem_Validos()
    {
        ...
        var conta = new Account("cliente1", 200, moeda, 100, 0);
        ...
        Assert.Equal("Solicitação de débito solicitada com sucesso!", resultado.Message);
    ...
    {
        // Arrange
        var conta = new Account("cliente2", 150, new Currency("Real", "BRL", 1), 100, 0);
        var command = new DebitCommand(
            Value: 100,
            Description: "Débito em moeda inválida",
            Currency: "USD"
        )
        {
            AccountId = Guid.NewGuid()
    ...
    public async Task Deve_Lancar_Excecao_Quando_Saldo_For_Insuficiente()
    {
        // Arrange
        var moeda = new Currency("Real", "BRL", 1);
        var conta = new Account("cliente3", 10, moeda, 0, 0); // saldo + crédito = 10
        
        var command = new DebitCommand(
            Value: 50,
            Description: "Débito acima do limite",
            Currency: "BRL"
        )
        {
            AccountId = Guid.NewGuid()
